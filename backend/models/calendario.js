...
exports.listarFechasHabiles = function (fn) {
    pool.query('call listar_fechas_habiles()', function (err, rows) {
        if (err) {
...
exports.listarFechasHabilesPorAnio = function (anio, fn) {
    pool.query('call listar_fechas_habiles_anio(' + anio + ')', function (err, rows) {
        if (err) {
            consulta = [{ 'codigo': 0, 'mensaje': "Error numero: " + err.errno + " descripcion: " + err.message }]
            fn(consulta);
        } else fn(rows[0]);
...
exports.modificarCalendario = function (arrayFechas, fn) {
    pool.query('call modificar_dias_habiles(' + arrayFechas + ')', function (err, rows) {
